...
export function WhyMe() {
  return (
    <section id="why-me" className="relative min-h-screen flex items-center py-24 px-6 bg-white" aria-labelledby="whyme-heading">
      <div className="max-w-5xl mx-auto w-full">
        ...
        >
          <h2 className="text-[28px] md:text-[36px] font-bold text-slate-900 tracking-tight mb-3" id="whyme-heading">
            为什么我适合这个岗位
          </h2>
          ...
          {cards.map((card, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -3 }}
                className={`group rounded-2xl p-5 border transition-all duration-500 hover:shadow-[0_8px_30px_-10px_rgba(0,0,0,0.08)] ${
                  card.highlight ? 'bg-brand-50/40 border-brand-100' : 'bg-[#fafafa] border-slate-100'
                }`}
              >
                <div className={`text-[10px] font-medium tracking-widest uppercase mb-3 ${card.highlight ? 'text-brand-500' : 'text-slate-300'}`}>
                  {card.category}
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[13px] font-semibold text-slate-900 mb-0.5">{card.title}</h3>
                    <div className={`text-[11px] mb-1.5 ${card.highlight ? 'text-brand-500' : 'text-slate-400'}`}>{card.subtitle}</div>
