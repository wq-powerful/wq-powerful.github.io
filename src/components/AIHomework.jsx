...
export function AIHomework() {
  return (
    <section id="ai-homework" className="relative min-h-screen flex items-center py-24 px-6 bg-white overflow-hidden" aria-labelledby="ai-hw-heading">
      {/* 背景装饰 */}
      ...
      <div className="max-w-5xl mx-auto w-full relative">
        ...
        >
          ...
          <h2 className="text-[30px] md:text-[40px] font-bold text-slate-900 tracking-tight" id="ai-hw-heading">
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="bg-[#fafafa] border border-slate-100 border-t-0 rounded-b-2xl p-4 h-full flex flex-col hover:shadow-[0_12px_40px_-12px_rgba(0,0,0,0.08)] transition-all duration-500">
                  {/* 序号 + 图标 */}
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-bold text-slate-300">{step.number}</span>
                    <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${step.color} flex items-center justify-center text-white opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300`}>
                      <Icon className="w-3.5 h-3.5" />
                    </div>
                  </div>

                  <h3 className="text-[13px] font-semibold text-slate-900 mb-2">{step.title}</h3>
                  <p className="text-[11px] text-slate-400 leading-[1.6] mb-3 flex-1">{step.desc}</p>

                  <div className="flex flex-wrap gap-1">
                    {step.tags.map((tag) => (
                      <span key={tag} className="px-2 py-0.5 bg-white text-slate-400 text-[9px] font-medium rounded-md border border-slate-100">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-slate-900 max-w-3xl mx-auto"
        >
          ...
          <div className="relative px-8 py-6 flex items-center gap-5">
            ...
            <div>
              <div className="text-[11px] text-brand-400 font-medium tracking-widest uppercase mb-1.5">我的 AI 使用原则</div>
